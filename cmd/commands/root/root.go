...
import (
	...
	"github.com/danielpickens/mobiusHPC/commands"
	"github.com/danielpickens/mobiusHPC/compute"
...
func runRootCommand(ctx context.Context, c Opts) error {
	...
	xklog.L = klogv2.New(vklog.Fields{})
	...
	/*---------------------------------------------------
	 * Discovery Portion of Kubernetes DNS server
	 *---------------------------------------------------*/
	{
	 ...
	 *---------------------------------------------------*/
	{
		...
		pc, err := node.NewPodController(node.PodControllerConfig{
			...
			EventRecorder:                        eb.NewRecorder(scheme.Scheme, corev1.EventSource{Component: "mobius-controller"}),
			Provider:                             virtualk8s,
	 ...
	 *---------------------------------------------------*/
	{
		...
		DefaultLogger.Info("... mobius is successfully initialized and waiting for jobs....")

		// wait for as long the app is running
		<-nc.Done()

		DefaultLogger.Info("... mobius has been gracefully terminated ....")
	}
...
func setNodeReady(n *corev1.Node) {
	for i, c := range n.Status.Conditions {
		...
		c.Message = "mobius is successfully connected to Slurm"
...
// Taint effect is read from the environment where the value is set.
// The taint key/value may be overwritten by the environment if the corresponding environment variable is set.
