...
webhooks:
  - name: "pod-mutator.MobiusHPC.dev"
    rules:
      - apiGroups:   [""]
        apiVersions: ["v1"]
        operations:  ["CREATE"]
    ...
    timeoutSeconds: 5
    sideEffects: None
  - name: "pvc-mutator.MobiusHPC.dev"
    rules:
      - apiGroups:   [""]
        apiVersions: ["v1"]
        operations:  ["CREATE"]
...
build: ## Build MobiusHPC binary
	GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build $(VERSION_FLAGS) -ldflags '-extldflags "-static"' -o bin/MobiusHPC-kubelet ./cmd/MobiusHPC

build-race: ## Build MobiusHPC binary with race condition detector
	GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build $(VERSION_FLAGS) -race -o bin/MobiusHPC-kubelet ./cmd/MobiusHPC
...
run-kubelet: ## Run the MobiusHPC Virtual Kubelet
	@echo "===> Generate MobiusHPC Certificates <==="
	mkdir -p ./bin

	if [ ! -f bin/kubelet.key ]; then openssl genrsa -out bin/kubelet.key 2048; fi

	echo "$$CERTIFICATE_CONFIGURATION" > bin/kubelet.cnf
	openssl req -new -key bin/kubelet.key -subj "/CN=MobiusHPC-kubelet" \
...
#.PHONY: build
#build: clean bin/MobiusHPC-kubelet

#.PHONY: clean
#clean: files := bin/MobiusHPC-kubelet
#clean:
