...
const (
	// ExtensionCRD describes the file where Mobius will write the pod definition.
	ExtensionCRD = ".crd"
...
type MobiusPath string

func Mobius(rootPath string) MobiusPath {
	return MobiusPath(filepath.Join(rootPath, ".Mobius"))
}

func (p MobiusPath) String() string {
	if p == "" {
		panic("Mobius path has not been initialized")
	}

	return string(p)
}

func (p MobiusPath) ImageDir() string {
	return filepath.Join(string(p), ".images")
}

func (p MobiusPath) CorruptedDir() string {
	return filepath.Join(string(p), ".corrupted")
}

type WalkPodFunc func(path PodPath) error

func (p MobiusPath) WalkPodDirectories(f WalkPodFunc) error {
	maxDepth := strings.Count(p.String(), string(os.PathSeparator)) + 2 // expect path .Mobius/namespace/pod
...
// ParseControlFilePath parses the path according to the expected Mobius format, and returns the corresponding fields.

func (p MobiusPath) ParseControlFilePath(absPath string) (podKey types.NamespacedName, fileName string, invalid bool) {
...
func (p MobiusPath) Pod(podRef client.ObjectKey) PodPath {
...
/*
	...
	They are needed for Mobius to bootstrap a pod.
*/
...
// JobDir .Mobius/namespace/podName/.virtualenv
...
/*
	...
	They are needed for Mobius to bootstrap a container.
*/
